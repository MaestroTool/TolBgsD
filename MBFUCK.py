...
def tampilhasil(akun,sandi,data):
	...
	i = inputD('[?] Tidak Puas ajing? Mau coba lagi skuy (y/t)',['Y','T'])
	if i.upper() == 'Y':
		return crack(data)
	else:
		return menu()
...
def inputD(x,v=0):
	while 1:
		...
		if v:
			...
			if len(a) == 0:
				cetak('!m[!] Masukan dengan benar ajing!')
				continue
			else:
				break
	return a
...
def login():
	...
	cetak('!h[*] Sedang Login sabar Tod...')
...
def idgroup():
	if log != 1:
		cetak('!h[*] Login !bFB!h dulu Kontol...')
		login()
		if log == 0:
			keluar()
	...
	while 1:
		...
		try:
			...
			cetak('!m[!] Mengambil !h %d id'%len(id_bgroup))
			break
	simpan()
	i = inputD('[?] Crack (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def idteman():
	if log != 1:
		cetak('!h[*] Login !bFB !hdulu Kontol...')
		login()
		if log == 0:
			keluar()
	cetak('!h[*] Mengumpulkan id teman...')
	...
	try:
		...
		if len(id_teman) != 0:
			cetak('!m[!] Mengambil !p%d id'%len(id_bteman))
		else:
			cetak('!m[!] Batal')
			keluar()
	while 1:
		...
		try:
			...
			cetak('\n!m[!] Mengambil !p%d id'%len(id_bteman))
			break
	simpan()
	...
	if i.upper() == 'Y':
		...
		return menu()
def menu():
	cetak(" !m* !bMULTI BRUTEFORCE FACEBOOK\n !m* !cMaestro\n !m* !phttps://github.com/mrb4ngs4d\n !m* !phttps://facebook.com/maestro10001\n !m !p\n!k\n|!h  Crack !mPASS!h DARI...  !k|\n''\n!k!p1. !hDAFTAR TEMAN\n!k!p2. !hANGGOTA GROUP\n!k!p3. !mKELUAR...")
